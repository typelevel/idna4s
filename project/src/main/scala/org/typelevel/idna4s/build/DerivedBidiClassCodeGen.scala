...
private[build] object DerivedBidiClassCodeGen {
  ...
  private final val BaseName = "BidirectionalClass"
  private final val BaseTypeName: String = s"${BaseName}Base"
  private final val GeneratedTypeName: String = s"Generated${BaseName}"
  ...
  def generate(dir: File, unicodeVersion: UnicodeVersion): Either[Throwable, File] = {
    ...
    bidiMappingFromUnicodeVersion(unicodeVersion).flatMap(mapping =>
      generateSource(mapping).leftMap(
        errors => s"""Unable to generate source file due to ${errors.mkString_(", ")}"""
      )
    ).leftMap(
  ...
  /** Newtype for bidirectional alias values.
    *
    * For example, for the bidirectional class named "Left_To_Right", the
    * `BidiAlias` string is "L".
    */
  ...
  /** Regular expression used to extract the bidi alias when parsing the bidi
    * name to bidi alias mapping.
    */
  private val bidiPropertyValueAliasRegex: Regex =
    """\s*bc\s*;\s*(\w+)\s*;\s*(\w+).*""".r

  /** Create the [[BidiName]] to [[BidiAlias]] mapping for the given version of
    * Unicode.
    */
  def bidiPropertyValueAlisesForUnicodeVersion(unicodeVersion: UnicodeVersion): Either[String, SortedMap[BidiName, BidiAlias]] =
    Either.catchNonFatal(
      URI.create(s"https://www.unicode.org/Public/${unicodeVersion.asString}/ucd/PropertyValueAliases.txt").toURL()
    ).leftMap(_.getLocalizedMessage).flatMap(
      bidiPropertyValueAlisesFromURL
    )

  /** Create the [[BidiName]] to [[BidiAlias]] mapping for the given URL which
    * should point to a version of the Unicode character data file named
    * "PropertyValueAliases.txt".
    */
  def bidiPropertyValueAlisesFromURL(url: URL): Either[String, SortedMap[BidiName, BidiAlias]] =
    Either.catchNonFatal(
      IO.readLinesURL(url)
    ).leftMap(_.getLocalizedMessage).flatMap((lines: List[String]) =>
      bidiPropertyValueAliasesFromLines(lines)
    )

  /** Create the [[BidiName]] to [[BidiAlias]] mapping for some `Foldable` set
    * of lines. These lines should represent some version of the Unicode
    * character data file "PropertyValueAliases.txt".
    */
  def bidiPropertyValueAliasesFromLines[F[_]: Foldable](lines: F[String]): Either[String, SortedMap[BidiName, BidiAlias]] =
  ...
  /** Create the mapping between [[CodePointRange]] values for a given version of Unicode.
    *
    * @note This method will download ''two'' files. Some of the bidirectional
    *       mappings are only defined in terms of the `@missing` annotated
    *       comments. These comments describe the mapping in terms for the
    *       [[BidiName]], however the normal body of the
    *       "DerivedBidiClass.txt" file uses the [[BidiAlias]]. For this
    *       reason, we have to download the mapping from [[BidiName]] to
    *       [[BidiAlias]] so that we can map the `@missing` values to
    *       [[BidiAlias]] values.
    *
    * @see [[https://www.unicode.org/reports/tr44/#Missing_Conventions]]
    */
  def bidiMappingFromUnicodeVersion(unicodeVersion: UnicodeVersion): Either[String, SortedMap[CodePointRange, BidiAlias]] =
    Either.catchNonFatal(
      URI.create(s"https://www.unicode.org/Public/${unicodeVersion.asString}/ucd/extracted/DerivedBidiClass.txt").toURL()
    ).leftMap(_.getLocalizedMessage).flatMap(url =>
      bidiPropertyValueAlisesForUnicodeVersion(unicodeVersion).flatMap(nameToAlias =>
        bidiMappingFromURL(url, nameToAlias)
      )
    )

  /** Create the mapping between [[CodePointRange]] values for a given URL which
    * should point to a version of the Unicode character data file
    * "DerivedBidiClass.txt".
    *
    * In order to handle the `@missing` annotated classes, a mapping between
    * [[BidiName]] and [[BidiAlias]] values is also required.
    *
    * @see [[https://www.unicode.org/reports/tr44/#Missing_Conventions]]
    */
  def bidiMappingFromURL(url: URL, nameToAlias: Map[BidiName, BidiAlias]): Either[String, SortedMap[CodePointRange, BidiAlias]] =
    Either.catchNonFatal(
      IO.readLinesURL(url)
    ).leftMap(_.getLocalizedMessage).flatMap((lines: List[String]) =>
      bidiMappingFromLines(lines, nameToAlias)
    )

  /** Create the mapping between [[CodePointRange]] values for a `Foldable` of
    * lines which should represent a version of the Unicode character data
    ...
    * @see [[https://www.unicode.org/reports/tr44/#Missing_Conventions]]
    */
  def bidiMappingFromLines[F[_]: Foldable](fa: F[String], nameToAlias: Map[BidiName, BidiAlias]): Either[String, SortedMap[CodePointRange, BidiAlias]] =
    (fa.foldM((Chain.empty[(CodePointRange, BidiName)], (Chain.empty[(CodePointRange, BidiAlias)]))){
      ...
      case (missingLines, normalLines) =>
        missingLines.foldM(Chain.empty[(CodePointRange, BidiAlias)]){
          case (acc, (k, v)) =>
            nameToAlias.get(v).fold(
              Left(s"Missing Bidi name to alias mapping for $v."): Either[String, Chain[(CodePointRange, BidiAlias)]]
            )(alias =>
              Right(acc :+ (k -> alias))
            )
      ...
      if (overlapping.isEmpty) {
        ...
        Left(s"""Overlapping BIDI ranges found, this is a bug: ${overlapping.mkString(", \n")}""")
      }
    }
  ...
  /** Regex used to parse an @missing line for a single code point. Note,
    * Unicode 15.0 doesn't have any instances of this but it is possible
    * future Unicode revisions will.
    */
  private val bidiMissingLineSingleRegex: Regex =
    """\s*#\s*@missing:\s*(\p{XDigit}{4,})\s*;\s*(\S+).*""".r

  /** For a line which is assumed to be a comment indicating an @missing
    * annotation, attempt to parse it as a [[CodePointRange]] and a
    * [[BidiName]].
    */
  ...
  /** For a line which is assumed to be a bidi mapping line, attempt to parse it
    * as a [[CodePointRange]] and [[BidiAlias]].
    */
  ...
  /** Attempt to generate the `Defn` values for the members of
    * `GeneratedBidirectionalClass`.
    */
  def bidiClassMappingDef(value: SortedMap[CodePointRange, BidiAlias]): Either[NonEmptyChain[String], List[Defn]] = {
    value.toVector.foldMap{
      case (single: CodePointRange.Single, value) =>
        (SortedMap(single -> NonEmptyChain.one(value)), SortedMap.empty[CodePointRange, NonEmptyChain[BidiAlias]])
      case (range, value) =>
        (SortedMap.empty[CodePointRange.Single, NonEmptyChain[BidiAlias]], SortedMap(range -> NonEmptyChain(value)))
    } match {
      case (singles, ranges) =>
        (flattenValuesOrError(singles).leftMap(NonEmptyChain.one), flattenValuesOrError(ranges).leftMap(NonEmptyChain.one)).parMapN{
          case (singles, ranges) =>
  ...
  /** Attempt to generate the source file tree for
    * "GeneratedBidirectionalClass.scala"
    */
  def generateSource(value: SortedMap[CodePointRange, BidiAlias]): Either[NonEmptyChain[String], Tree] =
...
private[idna4s] trait ${Type.Name(GeneratedTypeName)} extends ${Init(Type.Name(BaseTypeName), scala.meta.Name(""), Nil)} {
